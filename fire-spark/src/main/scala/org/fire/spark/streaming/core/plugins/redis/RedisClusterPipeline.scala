...
class RedisClusterPipeline(val sparkConf: SparkConf) {
  ...
  def cmdExec(redisCmd: RedisCmd, pipe: Pipeline): Unit = {
    redisCmd.cmd.toUpperCase match {
      ...
      case "HSET" => pipe.hset(redisCmd.key,redisCmd.params(0),redisCmd.params(1))
      case "EXPIRE" => pipe.expire(redisCmd.key, redisCmd.params(0).toInt)
      case "SADD" => pipe.sadd(redisCmd.key,redisCmd.params(0))
      case "ZADD" => pipe.zadd(redisCmd.key,redisCmd.params(0).toInt,redisCmd.params(1))
      case _ => //No-op
    }
  }
  ...
  def sadd(k:String,v:String):Unit={
    putCmd("sadd",k,Array(v))
  }

  def zadd(k:String,score:Int,v:String):Unit={
    putCmd("zadd",k,Array(score.toString,v))
  }

  def hset(k:String,fieldK:String,fieldV:String):Unit={
    putCmd("hset",k,Array(fieldK,fieldV))
  }
  def del(k:String):Unit={
    putCmd("del",k,Array.empty[String])
  }
